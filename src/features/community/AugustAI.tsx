...
// Brand aliases — maps user shorthand to the actual search keyword
...
function CouponChip({ coupon }: { coupon: AugustCoupon }) {
  return (
    ...
    >
      ...
      <span
        className={`shrink-0 rounded-full px-2 py-0.5 font-mono font-bold text-[10px] ${
          coupon.type === 'FREE'
            ? 'bg-brand-light text-brand-dark'
            : 'bg-stamp-light text-stamp-dark'
        }`}
      >
        {coupon.type === 'FREE'
          ? 'FREE'
...
function AugustChat({
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      role: 'assistant',
      content:
        "Hi! I'm August 👋 I help you find deals on CouponHub and answer your questions. Try asking about Swiggy, Zomato, Amazon deals — or how anything works!",
    },
  ]);
  ...
  useEffect(() => {
    ...
    const coupons: AugustCoupon[] =
      couponData.data.content.map((c) => ({
        id: c.id,
        title: c.title,
        brand: c.brand.name,
  ...
  return (
    <div
      className="flex flex-col overflow-hidden rounded-2xl border-2 border-line bg-paper shadow-2xl"
      style={{
        width: 320,
        height: 440,
      }}
    >
      ...
      <div className="flex shrink-0 items-center justify-between border-b-2 border-line bg-brand px-4 py-3">
        <div className="flex items-center gap-2.5">
          <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-white/20 font-display text-sm font-bold text-paper">
            A
          </div>

          <div>
            <p className="text-sm font-bold leading-none text-paper">
              August
            </p>

            <p className="mt-0.5 text-[11px] text-paper/70">
              CouponHub AI · Ask me anything
            </p>
          </div>
        </div>
      ...
      <div className="flex-1 space-y-3 overflow-y-auto p-3">
        {messages.map((msg, i) => (
          ...
          >
            {msg.role === 'assistant' && (
              <div className="mb-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-brand font-display text-[10px] font-bold text-paper">
                A
              </div>
            )}
            ...
            <div className="max-w-[82%] space-y-1.5">
              ...
              {msg.coupons &&
                msg.coupons.length > 0 && (
                  <div className="space-y-1">
                    <p className="ml-1 text-[11px] font-semibold text-ink-soft">
                      Found on CouponHub:
                    </p>
              ...
              {msg.coupons?.length === 0 && (
                <p className="ml-1 text-[11px] text-ink-soft">
                  No matching coupons right now.
                </p>
              )}
            </div>
          </div>
      ...
      <div className="shrink-0 border-t-2 border-line p-3">
        ...
        <p className="mt-1.5 text-center text-[10px] text-ink-soft/50">
          Powered by AI · August knows CouponHub
        </p>
      </div>
    </div>
  );
  ...
  return (
    ...
    >
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="absolute bottom-0 right-[calc(100%+12px)]"
          >
            <AugustChat
      ...
      <motion.button
        ...
        style={{
          background:
            'linear-gradient(135deg, #2F6B4F 0%, #1a4530 100%)',
        }}
        aria-label={
          open
            ? 'Close August AI'
